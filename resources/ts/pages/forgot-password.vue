...
const link = ref<Record<string, string | undefined>>({
  sent: undefined
})
const email = ref('')
...
const forgot = () => {
    ...
    axios.post('/api/password/email', { email: email.value })
    .then(r => {
        console.log(r);
        link.value.sent = r.data.message;
    })
    .catch(e => {
        ...
        <VCardText>
          <VForm 
            ...
            <VRow>
              <VCol cols="12">
                <div>
                  <VAlert variant="outlined" color="success" v-if="link.sent">
                    {{ link.sent }}
                  </VAlert>
              </div>
              </VCol>
