...
const refOffenceAddForm = ref<VForm>();
...
legislationListStore.fetchLegislationItems({
  ...
  englishLegislationList.value = filter(response.data.data, 'english');
  welshLegislationList.value = filter(response.data.data, 'welsh');
}).catch(e => {
  const { message } = e.response.data;
  alertMessage.value=message
  alertType.value='error'
...
const onSubmit = () => {
  refOffenceAddForm.value?.validate().then(({ valid: isValid }) => {
      if (isValid) {
        isTableLoading.value = true;
        offenceListStore.addOffence(offenceData.value).then(response => {
...
<template>
    <div>  
      <VCard title="Add Offence" class="mb-6">
        <VForm class="mt-2" ref="refOffenceAddForm" @submit.prevent="onSubmit">
          <VCardText>            
            <VRow>                
              <VCol md="6" cols="12">
                <VTextField
          ...
          <VCardText class="d-flex gap-4">
            <VBtn
              ...
              type="submit"
              @click="refOffenceAddForm?.validate()"
            >Save</VBtn>
